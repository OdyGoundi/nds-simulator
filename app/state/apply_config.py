...
def apply_static_config_to_state(cfg: Dict[str, object]) -> None:
    ...
    if system_key == "lorenz":
        ...
        st.session_state["sigma"] = to_float((params or {}).get("sigma", 10.0), 10.0)
        st.session_state["rho"] = to_float((params or {}).get("rho", 28.0), 28.0)
        st.session_state["beta"] = to_float((params or {}).get("beta", 8.0 / 3.0), 8.0 / 3.0)
    elif system_key == "rossler":
        params = system_obj.get("params") if isinstance(system_obj.get("params"), dict) else {}
        st.session_state["ross_a"] = to_float((params or {}).get("a", 0.2), 0.2)
        st.session_state["ross_b"] = to_float((params or {}).get("b", 0.2), 0.2)
        st.session_state["ross_c"] = to_float((params or {}).get("c", 5.7), 5.7)
    elif system_key == "henon_heiles":
        params = system_obj.get("params") if isinstance(system_obj.get("params"), dict) else {}
        st.session_state["hh_lambda"] = to_float((params or {}).get("lambda", 1.0), 1.0)
    ...
    if isinstance(solve_opts, dict):
        if "rtol" in solve_opts:
            st.session_state["rtol"] = to_float(solve_opts.get("rtol"), 1e-6)
        if "atol" in solve_opts:
            st.session_state["atol"] = to_float(solve_opts.get("atol"), 1e-8)
    ...
    if system_key == "henon_heiles":
        ...
        n_vars_axes = max(
            1,
            to_int(st.session_state.get("n_vars_sidebar", 0), 0),
            var_names_count,
        )
    ...
    if isinstance(plots_obj, dict):
        ...
        st.session_state["phase_x_idx_tab1"] = clamp_int(x_idx_cfg, 0, n_vars_axes - 1)
        st.session_state["phase_y_idx_tab1"] = clamp_int(y_idx_cfg, 0, n_vars_axes - 1)
        st.session_state["phase_z_idx_tab1"] = clamp_int(z_idx_cfg, 0, n_vars_axes - 1)
